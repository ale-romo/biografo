...
const Form =({ updateAction, deleteAction, method, item, id, users, videos, objects}) => {
    ...
    const handleDelete = useCallback( () => {
        fetch(`${deleteAction}&id=${id}`, {
            method: 'GET',
          }).then(() =>  {alert(`Elemento con ID #${id} eliminado.`);})
    },[ID, setID]);
    ...
    for(let fieldName in item){
        ...
        if (fieldName.includes('name')){
            ...
            inputs.push(<div className={styles.fieldsDiv} key={`${fieldName}.${item[fieldName]}`}>
    ...
    return <>
        <form action={updateAction} method={method} className='formsDiv'> 
            <>
            {inputs.map((input) => {return <> {input}</>})}
            <div className={styles.buttonsDiv}><button className={styles.linkButton}>Actualizar</button> <button className={styles.linkButtonWarning} onClick={handleDelete} type='button'>Borrar</button></div> <hr />
            </>
        </form>
        <hr />
    </>
